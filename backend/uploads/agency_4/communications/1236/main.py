from fastapi import FastAPI, Depends, HTTPException, Header, UploadFile, File, Form, status, Query
...
def list_all_payouts(
    ...
    ).join(models.Property).filter(models.Property.agency_id == agency_id)
...
def quick_collect(tenancy_id: int, amount: float, reference: str = "Manual Collection", db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    for plan in plans:
        ...
        fee = (gross_for_month * (Decimal(str(tenancy.management_fee_percentage)) / 100)) if tenancy.management_fee_percentage else Decimal('0')
...
async def get_dashboard_emails(agency_id: int, db: Session = Depends(get_db)):
    # Sync inbound emails in background conceptually, but we must await to avoid DB session detached errors
    await mail_service.sync_inbound_emails(db, agency_id)
...
from app.routers import landlord
